...
export class ComprobantesListaComponent implements OnInit {
  ...
  getDetalles(busqueda, page, size, columna, order){
    ...
    this._comprobantesListaService.getPedidosDetalles(this.body, busqueda, page, size, columna, order).subscribe(
      data => {
  ...
  buscar(busqueda){
    this._comprobantesListaService.getPedidoDetalle(this.body, busqueda, this.page, this.size, this.columna, this.order).subscribe(
      data => {
        this.dataSource2 = data.datos;
        this.length = data.totalRegistros;
        console.log(data.totalRegistros);
